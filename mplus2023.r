...
# 2017-08-09 Fix  for multiple labels in mplus.plot.loop.  Also add more arguments for customizations.
...
# 2023-11-04 Add the following functions to change the title, line width, legend text, legend location, legend text size, and legend box color for mplus.plot.loop (added by Erinn Acland):
#             title
#             lwid
#             leg.txt
#             leg.loc
...
#
mplus.plot.adjusted_estimated_means <-function(file,dfcovariates,procstr='process1',ptype='o') {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.plot.estimated_medians_with_percentiles <-function(file,procstr='process1',ptype='l') {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.plot.loop <- function(file,label=1,showgrid=TRUE,ylim,linecolors,linetype,noci=FALSE,xlabel,ylabel,title,lwid, 
                            leg.txt, leg.loc,leg.cex,leg.b.col 
                            ) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
  ...
  if (missing(title)) {
	...
	if (length(label) > 1) {
		...
		plot(xx,yy,xlab=xlabel,ylab=ylabel,main=cstr, frame.plot=F,
		     type='n',ylim=ylim)
		...
		for (r in c(1:num_loop)) {
			lines(xx[3*(r-1)+1,],yy[3*(r-1)+1,],col=linecolors[r], lwd=lwid)#, pch=plotchar[r])
		...
		if (missing(leg.txt)) {
		  leg.txt <- c("High Mod.","Low Mod.")
		}
		if (missing(leg.b.col)) {
		  leg.b.col <- "black"
		}
		
		legend(leg.loc,leg.txt,#title=leg.title,
		       #ldesc,
		      col=linecolors,lty=lty,lwd =lwid, cex=leg.cex,bty="o", box.col = leg.b.col)
	...
	plot(xx,yy,xlab=xlabel,ylab=ylabel,main=title,type='n',ylim=ylim)
...
mplus.plot.moderation <- function(file,label=1,group=1,allgroups=FALSE,showgrid=TRUE,lloc="top") {
	...
	if (allgroups && ngroups > 1) {
		...
		for (gidx in c(1:ngroups)) {
			lines(xx[3*(gidx-1)+1,],yy[3*(gidx-1)+1,],col=colors[gidx])
